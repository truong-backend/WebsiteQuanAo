...
// ─── Inner component that holds the stateful drawer/dropdown logic.
// It receives `locationKey` as a prop — when the route changes the parent
// passes a new key, which remounts this component and naturally resets all
// state to its initial values (mobileOpen: false, openDropdown: null).
...
const NavbarInner: React.FC<NavbarInnerProps> = ({ categories, scrolled, cartCount }) => {
  ...
  // ── FIX: removed the useEffect that called setMobileOpen / setOpenDropdown
  // synchronously on location change. The parent now passes `location.pathname`
  // as the `key` prop on <NavbarInner>, so a route change remounts the inner
  // component and resets state for free — no effect needed.
...
const Navbar: React.FC = () => {
  ...
  // Pass location.pathname as `key` so NavbarInner remounts on route change,
  // resetting mobileOpen and openDropdown without any useEffect setState call.
  return (
    <NavbarInner
