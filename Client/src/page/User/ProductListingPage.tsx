...
const ProductListingPage: React.FC = () => {
  ...
  // Đọc params từ URL (Navbar sẽ điều hướng về đây với ?category=... hoặc ?search=...)
  ...
  // Sync state khi URL thay đổi (khi Navbar điều hướng)
  useEffect(() => {
    ...
    setCategoryId(category ? Number(category) : undefined);
    setCurrentPage(0);
  ...
  // Fetch category options cho sidebar filter
  useEffect(() => {
  ...
  const handleReset = () => {
    ...
    setMinPrice(0); setMaxPrice(10000000);
  };

  // Tên danh mục đang active (hiển thị trên header)
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "grey.50" }}>
      {/* Navbar nằm ở đây — gọi API navbar 1 lần duy nhất */}
      <Navbar />
          ...
          <Grid container spacing={3}>
            ...
            <Grid size={{ xs: 12, md: 3 }}>
              <Paper elevation={2} sx={{ p: 3, position: "sticky", top: 80 }}>
                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
                  ...
                  <Button size="small" onClick={handleReset} sx={{ textTransform: "none" }}>
                    Xóa tất cả
                  </Button>
                </Box>
            ...
            <Grid size={{ xs: 12, md: 9 }}>
              ...
              {error && !loading && (
                <Alert
                  severity="error"
                  action={<Button color="inherit" size="small" onClick={fetchProducts}>Thử lại</Button>}
                >
                  {error}
                </Alert>
              )}
