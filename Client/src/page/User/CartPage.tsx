...
import { Box, Container, Typography, Paper, IconButton, Button, Divider, List, ListItem, ListItemAvatar, Avatar, ListItemText } from "@mui/material";
      ...
      <Container maxWidth="md">
        ...
        {items.length === 0 ? (
          ...
          <Paper sx={{ p: 3 }}>
            <List>
              {items.map((item) => (
                <ListItem key={item.id} secondaryAction={<IconButton onClick={() => handleRemove(item.id)}><Delete /></IconButton>}>
                  <ListItemAvatar>
                    <Avatar variant="rounded" src={`http://localhost:8080${item.img}`} alt={item.name} sx={{ width: 64, height: 64, mr: 2 }} />
                  </ListItemAvatar>
                  <ListItemText primary={item.name} secondary={`${item.price.toLocaleString("vi-VN")}₫ x ${item.quantity}`} />
