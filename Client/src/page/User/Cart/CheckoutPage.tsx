...
import {
  ...
  Grid as MuiGrid, FormControl, FormLabel, RadioGroup,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const order = await OrderService.createOrderFromCart(items, {
        phoneNumber, address, note: note || undefined,
      });
  ...
  if (!items.length) return (
    <Box sx={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      <Container maxWidth="md" sx={{ py: 4 }}>
        <Paper sx={{ p: 4 }}>
          <Typography>Giỏ hàng đang trống, hãy chọn sản phẩm trước.</Typography>
        </Paper>
      </Container>
      <Footer />
    </Box>
