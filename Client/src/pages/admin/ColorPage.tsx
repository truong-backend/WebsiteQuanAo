...
import { ColorService } from '@/modules';
import type { ColorResponse, ColorCreateRequest, ColorUpdateRequest } from '@/types';
...
const CREATE_FIELDS: FormField<ColorCreateRequest>[] = [
  { name: 'code', label: 'Mã màu',  type: 'text', placeholder: 'Nhập mã màu (#ffffff)', required: true },
...
function ColorFormCreate({ onSuccess }: { onSuccess?: () => void }) {
  return (
    <DynamicForm<ColorCreateRequest>
      ...
      onSubmit={(data) => ColorService.createColor({ code: data.code, name: data.name })}
      successMessage="Tạo màu thành công" onSuccess={onSuccess}
    />
  );
}

function ColorFormUpdate({ id, onSuccess }: { id: string; onSuccess?: () => void }) {
  const [initialData, setInitialData] = useState<ColorUpdateRequest | null>(null);
  ...
  return (
    <DynamicForm<ColorUpdateRequest>
      ...
      successMessage="Cập nhật màu thành công" onSuccess={onSuccess}
    />
  );
}
...
const COLUMNS: Column<ColorRecord>[] = [
  { key: 'code', label: 'Mã màu', sortable: true, searchable: true,
    render: (item) => (
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div
          style={{ width: 24, height: 24, borderRadius: 6, backgroundColor: item.code as string,
            border: '1px solid #e5e7eb', cursor: 'pointer' }}
          title="Click để copy"
          onClick={() => { navigator.clipboard.writeText(item.code as string); alert('Đã copy: ' + item.code); }}
        />
...
const ColorPage: React.FC = () => {
  ...
  const load = async () => {
    try {
      ...
      const res = await ColorService.getColorsPaged(0, 1000, undefined, 'code', 'asc');
  ...
  const handleDelete = async (item: ColorRecord) => {
    ...
    try { await ColorService.deleteColor(item.code); load(); }
    catch (err) { alert(err instanceof Error ? err.message : 'Có lỗi xảy ra khi xóa'); }
  };

  const handleEdit = (item: ColorRecord) => { setSelectedId(item.code as string); setShowUpdate(true); };
  ...
  if (loading && colors.length === 0) return <AdminPageState loading error={null} onRetry={load} />;
  if (error   && colors.length === 0) return <AdminPageState loading={false} error={error} onRetry={load} />;
  ...
  return (
    <div className={styles.page}>
      ...
      <DynamicList
        data={colors as ColorRecord[]} columns={COLUMNS} actions={actions}
        keyExtractor={(item) => item.code as string}
        emptyMessage="Không có dữ liệu" loading={loading} showGlobalSearch pageSize={10}
      />

      <AdminModal open={showCreate} title="Tạo mới" onClose={() => setShowCreate(false)}>
        <ColorFormCreate onSuccess={() => { setShowCreate(false); load(); }} />
      </AdminModal>
