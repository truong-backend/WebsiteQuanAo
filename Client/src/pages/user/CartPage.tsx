...
const CartPage: React.FC = () => {
  const [items, setItems] = useState<LocalCartItem[]>(() => LocalCartService.getCart());
  const navigate = useNavigate();

  const persist = (next: LocalCartItem[]) => { setItems(next); LocalCartService.setCart(next); };
                  ...
                  <div key={item.id}>
                    <div className={styles.item}>
                      <img className={styles.img}
                        src={`${BASE_URL}${item.img}`} alt={item.name}
