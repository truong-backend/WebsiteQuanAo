  ...
  return (
    <div className="flex flex-col gap-4 max-h-[70vh] overflow-y-auto pr-1">
      ...
      <div className="flex flex-col gap-1">
        ...
        >
          {BANNER_TYPES.map((t) => (
            <option key={t} value={t}>{BANNER_TYPE_LABEL[t]}</option>
          ))}
        </select>
      </div>
      ...
      {form.type === 'POPUP' && (
        <>
          <Input
            ...
            onChange={(e) => set('popupDelaySeconds', e.target.value ? Number(e.target.value) : undefined)}
          />
          <Input
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium uppercase tracking-wider text-brand-charcoal">Ngày bắt đầu</label>
          <input
            type="datetime-local"
            className="border border-brand-light px-3 py-2 text-sm focus:outline-none focus:border-brand-black"
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium uppercase tracking-wider text-brand-charcoal">Ngày kết thúc</label>
          <input
            type="datetime-local"
            className="border border-brand-light px-3 py-2 text-sm focus:outline-none focus:border-brand-black"
            value={form.endDate ?? ''}
...
export function AdminBanners() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess:  () => { invalidate(); setCreateOpen(false); toast('Đã tạo banner') },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess:  () => { invalidate(); setEditBanner(null); toast('Đã cập nhật banner') },
    onError:    () => toast('Cập nhật thất bại', 'error'),
  })

  const toggleMutation = useMutation({
    mutationFn: adminToggleBanner,
    onSuccess:  () => { invalidate(); toast('Đã thay đổi trạng thái') },
    onError:    () => toast('Thao tác thất bại', 'error'),
  })

  const deleteMutation = useMutation({
    mutationFn: adminDeleteBanner,
    onSuccess:  () => { invalidate(); toast('Đã xóa banner') },
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-light">
                {['', 'Ảnh', 'Tiêu đề', 'Loại', 'Ngày hết hạn', 'CTR', 'Trạng thái', ''].map((h) => (
              ...
              {filtered.map((banner, i) => (
                ...
                >
                  ...
                  <td className="py-3 pr-4">
                    ...
                    {banner.subtitle && (
                      <p className="text-[11px] text-brand-mid truncate max-w-[160px]">{banner.subtitle}</p>
                    )}
                    {banner.ctaText && (
                  ...
                  <td className="py-3 pr-4">
                    <button
                      ...
                      className={cn(
                        ...
                        banner.active ? 'bg-brand-black' : 'bg-brand-light',
                      )}
                    >
                      <span className={cn(
      ...
      {/* Create Modal */}
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Tạo banner mới">
        <BannerForm
          initial={EMPTY_FORM}
      ...
      {/* Edit Modal */}
      <Modal open={!!editBanner} onClose={() => setEditBanner(null)} title="Chỉnh sửa banner">
        {editBanner && (
          <BannerForm
            initial={{
