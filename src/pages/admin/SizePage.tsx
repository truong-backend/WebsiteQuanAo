...
import { SizeService } from '@/modules';
import type { SizesResponse, SizeCreateRequest, SizeUpdateRequest } from '@/types';
...
const CREATE_FIELDS: FormField<SizeCreateRequest>[] = [
  { name: 'id',   label: 'Mã kích thước',  type: 'text', placeholder: 'VD: S, M, L, XL', required: true },
  { name: 'name', label: 'Tên kích thước', type: 'text', required: true },
];

const UPDATE_FIELDS: FormField<SizeUpdateRequest>[] = [
  { name: 'name', label: 'Tên kích thước', type: 'text', placeholder: 'Nhập tên kích thước' },
];
...
function SizeFormCreate({ onSuccess }: { onSuccess?: () => void }) {
  return (
    <DynamicForm<SizeCreateRequest>
      ...
      onSubmit={(data) => SizeService.createSize({ id: data.id, name: data.name })}
      successMessage="Tạo kích thước thành công" onSuccess={onSuccess}
    />
  );
}

function SizeFormUpdate({ id, onSuccess }: { id: string; onSuccess?: () => void }) {
  ...
  return (
    <DynamicForm<SizeUpdateRequest>
      ...
      successMessage="Cập nhật kích thước thành công" onSuccess={onSuccess}
    />
  );
}
...
const SizePage: React.FC = () => {
  ...
  const handleDelete = async (item: SizeRecord) => {
    ...
    try { await SizeService.deleteSize(item.id); load(); }
    catch (err) { alert(err instanceof Error ? err.message : 'Có lỗi xảy ra khi xóa'); }
  };

  const handleEdit = (item: SizeRecord) => { setSelectedId(item.id as string); setShowUpdate(true); };
  ...
  if (loading && sizes.length === 0) return <AdminPageState loading error={null} onRetry={load} />;
  if (error   && sizes.length === 0) return <AdminPageState loading={false} error={error} onRetry={load} />;
  ...
  return (
    <div className={styles.page}>
      ...
      <DynamicList
        data={sizes as SizeRecord[]} columns={COLUMNS} actions={actions}
      ...
      <AdminModal open={showCreate} title="Tạo mới" onClose={() => setShowCreate(false)}>
        <SizeFormCreate onSuccess={() => { setShowCreate(false); load(); }} />
      </AdminModal>
