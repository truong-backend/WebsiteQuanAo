...
import {
  ...
  PopupBanner,
  CountdownBanner,
} from '@features/banner/ui/BannerComponents'
...
export default function HomePage() {
  ...
  // Parallax chỉ dùng khi fallback hero (ảnh tĩnh)
  useEffect(() => {
    const el = heroRef.current
    if (!el) return
    const handler = () => { el.style.backgroundPositionY = `${window.scrollY * 0.4}px` }
  ...
  return (
    <main>
      ...
      {/* ── Countdown flash sale (nếu có) ── */}
      <CountdownBanner />

      {/* ── Hero slider — fallback về hero tĩnh ── */}
      <HeroBannerSlider
        fallback={
          <section
            ref={heroRef}
          ...
          >
            ...
            <div className="relative z-10 text-center text-brand-white flex flex-col items-center gap-8 px-6 animate-fade-up">
              ...
              <h1 className="font-display text-6xl sm:text-8xl font-light leading-none">
                Thời Trang<br /><em className="italic">Cao Cấp</em>
              </h1>
      ...
      {/* ── Service strip — fallback về hardcode ── */}
      <ServiceBanners
        fallback={
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {categories.slice(0, 4).map((cat, i) => {
              ...
              return (
                ...
                >
                  <img src={imgSrc} alt={cat.categoryName} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                  ...
                  <div className="absolute inset-0 flex items-end p-6">
                    <div>
                      ...
                      <p className="text-xs uppercase tracking-widest text-brand-white/70 group-hover:text-brand-gold transition-colors mt-1">Xem tất cả →</p>
                    </div>
                  </div>
                </Link>
              )
      ...
      <section className="container mx-auto px-6 max-w-screen-xl py-20">
        <div className="flex items-end justify-between mb-12">
          ...
          <Link to={ROUTES.shop} className="text-xs uppercase tracking-widest text-brand-mid hover:text-brand-black transition-colors underline underline-offset-4">
            Xem tất cả
          </Link>
        </div>
      ...
      {/* ── Promotion banner — fallback về editorial tĩnh ── */}
      <PromotionBanner
        fallback={
            ...
            <div className="relative z-10 container mx-auto px-6 max-w-screen-xl">
              <div className="max-w-lg">
                ...
                <h2 className="font-display text-5xl text-brand-white font-light mb-6">
                  Sale đến<br /><span className="text-brand-gold">50%</span>
                </h2>
                ...
                <Link to={`${ROUTES.shop}?sortBy=basePrice&sortDir=asc`}>
                  <Button size="lg" className="bg-brand-gold text-brand-black border-0 hover:bg-brand-white">
                    Mua ngay
                  </Button>
                </Link>
              </div>
