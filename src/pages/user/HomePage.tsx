...
const TICKER_ITEMS = ['THỜI TRANG', 'PHONG CÁCH', 'CHẤT LƯỢNG', 'ĐỘC ĐÁO', 'HIỆN ĐẠI', 'FASHION', 'STYLE', 'TRENDY'];

const TickerRow: React.FC<{ reverse?: boolean; outlined?: boolean }> = ({ reverse = false, outlined = false }) => {
  const items = [...TICKER_ITEMS, ...TICKER_ITEMS, ...TICKER_ITEMS];
  return (
    <div className={styles.tickerRow}>
      <div className={[styles.tickerTrack, reverse ? styles['tickerTrack--reverse'] : styles['tickerTrack--forward']].join(' ')}>
        {items.map((item, i) => (
          <span key={i} className={[styles.tickerItem, outlined ? styles['tickerItem--outlined'] : ''].join(' ')}>
            {item}
          </span>
        ))}
      </div>
...
const StepCard: React.FC<{ num: string; title: string; desc: string; accent?: boolean }> = ({ num, title, desc, accent }) => (
  <div className={[styles.stepCard, accent ? styles['stepCard--accent'] : ''].join(' ')}>
    <div className={styles.stepCard__num}>{num}<span className={styles['stepCard__buoc']}>Bước</span></div>
    ...
    <div className={styles.page}>
      ...
      <section className={styles.hero} ref={heroRef}>
        ...
        <div className={styles.hero__left}
          style={{ opacity: heroReady ? 1 : 0, transform: heroReady ? 'translateY(0)' : 'translateY(28px)', transition: 'opacity 0.7s ease, transform 0.7s ease' }}>
          <p className={styles.hero__eyebrow}>Hãy làm quen <span className={styles.hero__dots}>● ● ●</span></p>
      ...
      <section className={styles.about}>
        ...
        <div className={styles.about__inner}>
          <div>
            ...
            <h2 className={styles.about__title}>Những Bộ Trang Phục<br /><em>Đỉnh Cao Phong Cách</em></h2>
            <p className={styles.about__text}>
              ShopVN mang đến hàng ngàn sản phẩm thời trang được tuyển chọn kỹ lưỡng —
              từ các thương hiệu trong nước đến quốc tế, với cam kết chất lượng và phong cách dẫn đầu xu hướng.
            </p>
          </div>
        ...
        <div className={styles.steps__inner}>
          <div className={styles.steps__header}>
            <div>
              ...
              <h2 className={styles.steps__title}>Trải Nghiệm Mua Sắm<br />Hoàn Hảo Của Bạn</h2>
            </div>
      ...
      <section className={styles.cta}>
        <div className={styles.cta__bgText} aria-hidden>
          {['FASHION', 'STYLE', 'TREND'].map((w, i) => (
            <span key={w} className={styles.cta__bgWord} style={{ top: `${20 + i * 30}%`, left: `${-5 + i * 30}%` }}>{w}</span>
          ))}
        </div>
        <div className={styles.cta__inner}>
          <p className={styles.cta__eyebrow}>Sẵn sàng chưa?</p>
          <h2 className={styles.cta__title}>Mặc Đẹp Hơn.<br /><span>Sống Chất Hơn.</span></h2>
          <p className={styles.cta__desc}>
            Hơn 50.000 khách hàng đã tin tưởng ShopVN. Tham gia ngay để nhận ưu đãi đặc biệt dành cho thành viên mới.
          </p>
          <button className={`${styles.btnPrimary} ${styles['btnPrimary--lg']}`} onClick={() => navigate('/products')}>
            Mua Sắm Ngay →
          </button>
        </div>
      </section>
