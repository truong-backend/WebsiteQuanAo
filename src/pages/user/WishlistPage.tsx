...
interface WishItem { id: string; name: string; price: number; category: string; img: string; }
...
const MOCK_ITEMS: WishItem[] = [
  {
    id: '1', name: 'Áo Khoác Dạ Atelier', price: 3_500_000, category: 'Outerwear',
  ...
  {
    id: '2', name: 'Váy Lụa Slip-on', price: 2_200_000, category: 'Dresses',
  ...
  {
    id: '3', name: 'Blazer Cấu Trúc', price: 4_800_000, category: 'Tailoring',
...
const WishlistPage: React.FC = () => {
  ...
  const handleAddToCart = (item: WishItem) => {
    LocalCartService.addItem({ id: item.id, name: item.name, price: item.price, img: item.img }, 1);
    navigate('/cart');
  };
          ...
          {NAV_LINKS.map((link) => (
            <Link key={link.label} to={link.to}
              ...
              <span className={styles.navIcon}
                style={link.active ? { fontVariationSettings: "'FILL' 1, 'wght' 300" } : undefined}>
                {link.icon}
              </span>
              {link.label}
            </Link>
      ...
      <main className={styles.main}>
        ...
        {items.length === 0 ? (
          <div className={styles.empty}>
            ...
            <p className={styles.emptyDesc}>Bạn chưa có sản phẩm nào trong danh sách yêu thích.</p>
            <Link to="/products" className={styles.emptyAction}>Tiếp tục mua sắm</Link>
          </div>
        ) : (
              ...
              <div key={item.id} className={styles.card}>
                ...
                <div className={styles.body}>
                  <div className={styles.nameRow}>
                    ...
                    <span className={styles.price}>{item.price.toLocaleString('vi-VN')}₫</span>
                  </div>
