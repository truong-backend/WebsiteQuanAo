...
const NavbarInner: React.FC<NavbarInnerProps> = ({ categories, scrolled, cartCount }) => {
      ...
      <div className={styles.inner}>
        <div className={styles.bar}>
          ...
          <nav className={styles.desktopLinks}>
            <Link
              to="/home"
              className={[styles.navLink, isActive('/') ? styles['navLink--active'] : ''].join(' ')}
            >
              Trang Chủ
            </Link>
            <Link
              to="/about"
              className={[styles.navLink, isActive('/about') ? styles['navLink--active'] : ''].join(' ')}
            >
              Về Chúng Tôi
            </Link>
            ...
            <Link
              to="/contact"
              className={[styles.navLink, isActive('/contact') ? styles['navLink--active'] : ''].join(' ')}
            >
              Liên Hệ
            </Link>
          </nav>
          ...
          <div className={styles.rightActions}>
            ...
            <button
              ...
              style={{ position: 'relative', background: 'none', color: '#3f3f46', padding: '0.25rem' }}
              onClick={() => navigate('/cart')}
              title="Giỏ hàng"
            >
              <CartIcon />
              {cartCount > 0 && (
                <span className={styles.drawerBadge} style={{ position: 'absolute', top: '-6px', right: '-6px' }}>
                  {cartCount}
                </span>
              )}
            </button>
            ...
            {isAuthenticated ? (
              <div className={styles.userMenu}>
                <button className={styles.userBtn} onClick={() => toggle('user')}>
                  <UserIcon /> &nbsp;{username}
                </button>
                {openDropdown === 'user' && (
      ...
      {mobileOpen && (
        <div className={styles.drawer}>
          ...
          <div className={styles.drawerLinks}>
            <button className={styles.drawerItem} onClick={() => { navigate('/'); setMobileOpen(false); }}>
              Trang Chủ
            </button>
            <button className={styles.drawerItem} onClick={() => { navigate('/about'); setMobileOpen(false); }}>
              Về Chúng Tôi
            </button>
            <button className={styles.drawerItem} onClick={() => { navigate('/products'); setMobileOpen(false); }}>
              Tất cả sản phẩm
            </button>
            {categories.map((cat) => (
              <button
            ...
            <button className={styles.drawerItem} onClick={() => { navigate('/contact'); setMobileOpen(false); }}>
              Liên Hệ
            </button>
          </div>
