...
// Chịu trách nhiệm: Form động dùng chung cho mọi entity admin
//   - Render field theo config: text | email | password | number | textarea | select | image | datetime-local
//   - Mode: create (reset sau submit) | update (load initialData hoặc gọi loadData())
//   - Upload ảnh lên /uploads/image, xóa ảnh qua DELETE /uploads/images?fileName=
//   - Load select options lazy qua loadOptions()
...
interface DynamicFormProps<T extends Record<string, unknown>> {
  fields:            FormField<T>[];
  // Promise<unknown> thay vì Promise<void> — services trả về entity, không phải void
