// ─────────────────────────────────────────────────────────────
//  modules/color/color.module.ts
//  Chịu trách nhiệm: CRUD màu sắc, select options
// ─────────────────────────────────────────────────────────────
...
class ColorApi extends BaseApi<ColorResponse, ColorCreateRequest, ColorUpdateRequest> {
  ...
  getAllColorOptions(): Promise<ColorOption[]> {
...
export const ColorService = {
  async getColorsPaged(
    page = 0, size = 10,
    search?: string,
    sortBy = "code", sortDir: "asc" | "desc" = "asc",
  ...
  async getColorById(id: string): Promise<ColorResponse> {
    try {
      return await colorApi.getById(id);
    } catch (e) {
      if (axios.isAxiosError(e) && e.response) {
