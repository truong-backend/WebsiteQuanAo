...
// Moved from: src/api/BaseApi/baseApi.ts
...
> {
  ...
  async getAll<TPageResponse>(
    page = 0, size = 10, search?: string, sortBy?: string,
    sortDir: "asc" | "desc" = "asc",
    additionalParams?: Record<string, string | number | boolean>
  ): Promise<TPageResponse> {
    const params: Record<string, string> = {
      page: page.toString(), size: size.toString(), sortDir,
    };
  ...
  async customGet<TCustomResponse>(path: string, config?: AxiosRequestConfig): Promise<TCustomResponse> {
    const response: AxiosResponse<TCustomResponse> = await this.axiosInstance.get(path, config);
    return response.data;
  }

  async customPost<TCustomResponse, TData = unknown>(path: string, data?: TData, config?: AxiosRequestConfig): Promise<TCustomResponse> {
    const response: AxiosResponse<TCustomResponse> = await this.axiosInstance.post(path, data, config);
    return response.data;
  }

  async customPut<TCustomResponse, TData = unknown>(path: string, data?: TData, config?: AxiosRequestConfig): Promise<TCustomResponse> {
    const response: AxiosResponse<TCustomResponse> = await this.axiosInstance.put(path, data, config);
    return response.data;
  }

  async customDelete<TCustomResponse>(path: string, config?: AxiosRequestConfig): Promise<TCustomResponse> {
...
export interface AdditionalParams { [key: string]: string | number | boolean | undefined; }
...
export interface PageResponse<T> {
  ...
  last: boolean; totalElements: number; totalPages: number;
  size: number; number: number;
  sort: { empty: boolean; sorted: boolean; unsorted: boolean };
  numberOfElements: number; first: boolean; empty: boolean;
}

export interface SortConfig { empty: boolean; sorted: boolean; unsorted: boolean; }
export interface PageableConfig {
  pageNumber: number; pageSize: number; sort: SortConfig;
  offset: number; paged: boolean; unpaged: boolean;
}
