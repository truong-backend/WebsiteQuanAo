// src/features/cart/components/CheckoutPage.tsx
// Moved from: src/pages/user/CheckoutPage.tsx
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentMethod === "VNPAY") {
        const url = await PaymentGatewayService.createVnpayPayment(
          order.id,
          total,
        );
      ...
      if (paymentMethod === "MOMO") {
        const url = await PaymentGatewayService.createMomoPayment(
          order.id,
          total,
        );
        LocalCartService.clear();
      ...
      setError(
        err instanceof Error ? err.message : "Không thể tạo đơn hàng mới",
      );
    } finally {
      setSubmitting(false);
    }
