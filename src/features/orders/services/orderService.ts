...
import type {
  OrderResponse, OrderResponsePageResponse,
  OrderCreateRequest, OrderUpdateRequest,
} from '../types/order.types';
import { OrderStatus } from '../types/order.types';
import type { CartItem } from '@/features/cart/services/localCartService';
...
export const OrderService = {
  async getOrdersPaged(
    ...
    status?: OrderStatus, startDate?: string, endDate?: string, accountId?: number,
  ...
  async createOrder(payload: OrderCreateRequest): Promise<OrderResponse> {
    try { return await orderApi.createUserOrder(payload); }
    catch (e) {
  ...
  async updateOrder(id: string, payload: OrderUpdateRequest): Promise<OrderResponse> {
    try { return await orderApi.update(id, payload); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
  ...
  async getOrderById(id: string): Promise<OrderResponse> {
    try { return await orderApi.getById(id); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
        if (e.response.status === 404) throw new Error('Không tìm thấy đơn hàng');
  ...
  async deleteOrder(id: string): Promise<void> {
    try { return await orderApi.delete(id); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
  ...
  async updateOrderStatus(id: string, status: OrderStatus): Promise<OrderResponse> {
    try { return await orderApi.updateStatus(id, status); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
