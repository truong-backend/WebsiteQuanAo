...
// Moved from: src/modules/category/category.module.ts (CategoryService)
import axios from 'axios';
import { categoryApi } from '../api/categoryApi';
import type {
  CategoryResponse, CategoryResponsePageResponse,
  CategoryCreateAndUpdateRequest, CategoryOption, NavbarCategory,
...
export const CategoryService = {
  async getCategoriesPaged(page = 0, size = 10, search?: string, sortBy = 'categoryId', sortDir: 'asc' | 'desc' = 'asc', parentId?: number): Promise<CategoryResponsePageResponse> {
    try { return await categoryApi.getAllCategories(page, size, search, sortBy, sortDir, parentId); }
  ...
  async createCategory(payload: CategoryCreateAndUpdateRequest): Promise<CategoryResponse> {
    try { return await categoryApi.createCategory(payload); }
    catch (e) {
  ...
  async updateCategory(id: number, payload: CategoryCreateAndUpdateRequest): Promise<CategoryResponse> {
    try { return await categoryApi.updateCategory(id, payload); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
        const m = (e.response.data as ErrorResponse).message;
  ...
  async getCategoryById(id: number): Promise<CategoryResponse> {
    try { return await categoryApi.getCategoryById(id); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
  ...
  async deleteCategory(id: number): Promise<void> {
    try { return await categoryApi.deleteCategory(id); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
