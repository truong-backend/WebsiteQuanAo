...
import { fetchActiveBanners, trackBannerClick, trackBannerImpression } from '../api/bannerApi'
...
function BannerLink({
  ...
  if (isExternal) {
    return (
      <a href={banner.linkUrl} target="_blank" rel="noopener noreferrer" className={className} onClick={handleClick}>
        {children}
      </a>
    )
  }
  ...
  return (
    <section className="relative h-[85vh] min-h-[600px] overflow-hidden bg-brand-black">
      ...
      <div className="relative z-10 h-full flex items-center justify-center">
        <BannerLink banner={banner} className="text-center text-brand-white flex flex-col items-center gap-8 px-6 animate-fade-up">
          {banner.subtitle && (
            <p className="text-[10px] uppercase tracking-[0.4em] text-brand-gold">{banner.subtitle}</p>
          )}
          <h1 className="font-display text-6xl sm:text-8xl font-light leading-none">{banner.title}</h1>
          {banner.ctaText && (
    ...
    <section className="relative h-[50vh] min-h-[400px] overflow-hidden bg-brand-charcoal flex items-center">
      ...
      <div className="relative z-10 container mx-auto px-6 max-w-screen-xl">
        <div className="max-w-lg">
          {banner.subtitle && (
            <p className="text-[10px] uppercase tracking-[0.4em] text-brand-gold mb-4">{banner.subtitle}</p>
          )}
          <h2 className="font-display text-5xl text-brand-white font-light mb-6">
            {banner.discountPercent ? (
              <>
                Sale đến<br />
                <span className="text-brand-gold">{banner.discountPercent}%</span>
              </>
            ) : (
              banner.title
            )}
    ...
    <section className="bg-brand-cream border-y border-brand-light">
      <div className="container mx-auto px-6 max-w-screen-xl">
        <ul
          ...
          style={{ gridTemplateColumns: `repeat(${banners.length}, minmax(0, 1fr))` }}
        >
          {banners.map((b) => {
            const handleClick = () => trackBannerClick(b.id).catch(() => {})
            return (
              <li key={b.id} className="flex flex-col items-center gap-2 py-8 px-4 text-center">
                <p className="text-xs font-medium uppercase tracking-wider">{b.title}</p>
                {b.subtitle && <p className="text-xs text-brand-mid">{b.subtitle}</p>}
              </li>
            )
          })}
        </ul>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-black/60 animate-fade-up">
      <div className="relative bg-brand-white max-w-md w-full mx-4 shadow-2xl">
        ...
        <img src={banner.imageUrl} alt={banner.title} className="w-full max-h-64 object-cover" />
        <div className="p-6 text-center">
          <h3 className="font-display text-2xl mb-2">{banner.title}</h3>
          {banner.subtitle && <p className="text-sm text-brand-mid mb-4">{banner.subtitle}</p>}
          {banner.voucherCode && (
            <div className="border border-dashed border-brand-gold px-4 py-2 mb-4 inline-block">
              <span className="text-xs uppercase tracking-widest text-brand-mid">Mã giảm giá: </span>
              <span className="font-mono font-bold text-brand-gold">{banner.voucherCode}</span>
            </div>
          )}
          {banner.ctaText && banner.linkUrl && (
            <BannerLink banner={banner}>
  ...
  return (
    <section className="bg-brand-black text-brand-white py-6">
      <div className="container mx-auto px-6 max-w-screen-xl flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          {banner.subtitle && (
            <p className="text-[10px] uppercase tracking-[0.4em] text-brand-gold mb-1">{banner.subtitle}</p>
          )}
          ...
          {[
            ...
            <div key={label} className="flex items-center gap-3">
              <div className="flex flex-col items-center">
                ...
                <span className="text-[10px] uppercase tracking-wider text-brand-white/50">{label}</span>
              </div>
              {i < 2 && <span className="text-brand-gold text-2xl font-light mb-3">:</span>}
            </div>
          ))}
        </div>
        {banner.ctaText && banner.linkUrl && (
