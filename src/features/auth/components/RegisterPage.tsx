...
// Moved from: src/pages/auth/RegisterPage.tsx
        ...
        <form onSubmit={handleSubmit} noValidate>
          {(["name", "email"] as const).map((field) => (
            <div key={field} className={styles.field}>
              <label className={styles.label}>{field === "name" ? "Họ và tên" : "Email"}</label>
              <div className={styles.inputWrap}>
                <span className={styles.inputIcon}>{field === "name" ? "👤" : "✉"}</span>
                <input
                  type={field === "email" ? "email" : "text"}
                  placeholder={field === "name" ? "Nguyễn Văn A" : "example@email.com"}
                  value={values[field]} onChange={(e) => set(field, e.target.value)} disabled={loading}
                />
              </div>
          ...
          {(["password", "confirmPassword"] as const).map((field) => {
            ...
            const toggle = field === "password" ? () => setShowPw((p) => !p) : () => setShowCp((p) => !p);
            return (
              <div key={field} className={styles.field}>
                <label className={styles.label}>{field === "password" ? "Mật khẩu" : "Xác nhận mật khẩu"}</label>
                <div className={styles.inputWrap}>
                  <span className={styles.inputIcon}>🔒</span>
                  <input
                    type={show ? "text" : "password"}
                    placeholder={field === "password" ? "Nhập mật khẩu" : "Nhập lại mật khẩu"}
                    value={values[field]} onChange={(e) => set(field, e.target.value)}
                    disabled={loading} style={{ paddingRight: 40 }}
                  />
                  <button type="button" className={styles.pwToggle} onClick={toggle}>
                    {show ? "🙈" : "👁"}
                  </button>
                </div>
                {errors[field] && <span className={styles.fieldError}>{errors[field]}</span>}
