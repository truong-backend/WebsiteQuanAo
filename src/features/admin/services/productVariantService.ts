// src/features/admin/services/productVariantService.ts
// Moved from: src/modules/productVariant/productVariant.module.ts (ProductVariantService)
import axios from 'axios';
import { productVariantApi } from '../api/productVariantApi';
import type {
  ProductVariantResponse, ProductVariantResponsePageResponse,
...
export const ProductVariantService = {
  async getProductVariantsPaged(page = 0, size = 10, search?: string, sortBy = 'id', sortDir: 'asc' | 'desc' = 'asc'): Promise<ProductVariantResponsePageResponse> {
    try { return await productVariantApi.getAll(page, size, search, sortBy, sortDir); }
  ...
  async createProductVariant(payload: ProductVariantCreateRequest): Promise<ProductVariantResponse> {
    try { return await productVariantApi.create(payload); }
    catch (e) {
  ...
  async updateProductVariant(id: string, payload: ProductVariantUpdateRequest): Promise<ProductVariantResponse> {
    try { return await productVariantApi.update(id, payload); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
        const m = (e.response.data as ErrorResponse).message;
  ...
  async getProductVariantById(id: string): Promise<ProductVariantResponse> {
    try { return await productVariantApi.getById(id); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
  ...
  async deleteProductVariant(id: string): Promise<void> {
    try { return await productVariantApi.delete(id); }
    catch (e) {
      if (axios.isAxiosError(e) && e.response) {
        const m = (e.response.data as ErrorResponse).message;
