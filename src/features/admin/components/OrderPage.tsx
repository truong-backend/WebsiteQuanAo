// src/features/admin/components/OrderPage.tsx
// Moved from: src/pages/admin/OrderPage.tsx
// Changed imports: @/modules → @/features/orders | @/types → @/features/orders
...
const buildColumns = (
  onStatusChange: (item: OrderResponse, newStatus: string) => void,
): Column<OrderResponse>[] => [
  {
    key: "id",
    label: "Mã đơn",
    render: (item) => (
      <span style={{ fontFamily: "monospace", fontSize: 13 }}>{item.id}</span>
    ),
  },
  {
    key: "orderTime",
...
const OrderPage: React.FC = () => {
  ...
  const handleStatusChange = async (item: OrderResponse, newStatus: string) => {
    try {
      await OrderService.updateOrderStatus(item.id, newStatus as OrderStatus);
      load();
    } catch (err) {
