// src/features/admin/components/ContactPage.tsx
// Moved from: src/pages/admin/ContactPage.tsx
// Changed imports: @/modules → ../services/contactService | @/types → ../types/contact.types
...
const ReplyForm = ({ contact, onSuccess }: ReplyFormProps) => {
  ...
  const [form, setForm] = useState<ContactReplyRequest>({
  ...
  return (
    <div className={styles.replyForm}>
      ...
      <div className={styles.field}>
        <label className={styles.label} htmlFor="reply-subject">
          Tiêu đề
        </label>
        <input
          id="reply-subject"
  ...
  const handleOpen = async (contact: ContactResponse) => {
    const updated =
      contact.status === "UNREAD"
        ? await ContactService.getById(contact.id)
        : contact;
    setSelected(updated);
    load();
  };
          ...
          <tbody>
            {contacts.length === 0 ? (
              ...
              contacts.map((c) => (
                <tr
                  key={c.id}
                  className={c.status === "UNREAD" ? styles.rowUnread : ""}
                >
