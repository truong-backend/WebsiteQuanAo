// src/features/admin/components/ProductPage.tsx
// Moved from: src/pages/admin/ProductPage.tsx
// Changed imports: @/modules → @/features/products + @/features/categories
...
function ProductFormUpdate({
  ...
  const [initialData, setInitialData] = useState<ProductUpdateRequest | null>(
    null,
  );
  ...
  if (!initialData)
    return (
      <div style={{ textAlign: "center", padding: "16px", color: "#ef4444" }}>
  ...
  },
  {
    ...
    render: (item) => (
      <img
        ...
        onError={(e) => {
          e.currentTarget.src = "https://via.placeholder.com/72?text=N/A";
        }}
      />
    ),
  },
...
const ProductPage: React.FC = () => {
  ...
  const handleDelete = async (item: ProductRecord) => {
    ...
    try {
      await ProductService.deleteProduct(item.id as string);
      load();
    } catch (err) {
